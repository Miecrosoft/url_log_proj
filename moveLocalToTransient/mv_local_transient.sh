...
fMOVE() {
	...
	if [ $PROBES != "rob" ]; then
		...
		echo "hadoop fs -moveFromLocal $i /data/transient/url_${PROBES}_${YEARS}${MONTHS}/ds=$YEARS-$MONTHS-$DAYS"
		...
		echo "hadoop fs -moveFromLocal $i /data/transient/url_${PROBESR}_${YEARSR}${MONTHSR}/ds=$YEARSR-$MONTHSR-$DAYSR"
